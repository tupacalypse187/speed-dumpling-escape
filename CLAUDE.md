...
| `levels.ts` | Infinite-level math (`levelThreshold`, ~2.2× growth past Lv 4, 2-sig-fig rounding), level colors/names, milestone skins, shared physics constants (`GRAV`, `JUMP_V`, `JUMP_AIRTIME`, `JUMP_HEIGHT`), world types (`ObbyDef`, `Platform`, `PadDef`, `MovingPlatformDef`, `Hazard`), hand-crafted Obbies 1–3. |
...
| `save.ts` | `SaveData` schema (versioned), `loadSave` with **graceful migration** from every older shape, `storeSave`, `fmtTime`. Same localStorage key across versions; migrate in place. |
